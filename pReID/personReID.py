...
class personReIdentifier(object):
    ...
    def PersonReIdentification_MT(self, query_path, cropps_path, out_reid_path, topN, show_query = False):
        print('RE-IDENTIFICATION...............')
        topN = topN - 1
        #GALLERY , donde se consultara!
        files = sorted(glob.glob( cropps_path+'/*'))
        print('cropps files: ',len(files))
        ...
        #tofos files - os IDs
        for carpet in files:
            carpet_files = sorted(glob.glob( carpet +'/*.png'))
